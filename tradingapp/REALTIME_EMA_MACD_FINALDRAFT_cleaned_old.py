...
class TradeApp(EWrapper, EClient): 
    ...
    def tickPrice(self, reqId, tickType, price, attrib):
        ...
        print("snap done")
        #temp_options_price[reqId] = [price]
        #temp_options_price[reqId].append(price)
...
def main():
    ...
    for ticker in tickers:
        ...
        def placeorder_func(type:str):
            ...
            time.sleep(4)
            print("test2.5")
            print(find_conid(type, 1))
            print(find_conid(type, 4))
            print(optionSpread(ticker, conid1 = find_conid(type, 1), conid2 = find_conid(type, 4)).comboLegs)
        ...
        if len(pos_df)==0:
            if (df["Close"][-1]> df["BB UP"][-1] and df["EMA 21"][-1]> df["EMA 55"][-1]) or df["Close"][-1]>50:
                placeorder_func("P")
